document.addEventListener('DOMContentLoaded', () => {
  const translations = {
    en: {
      ...
      autoLockUnit: "min",
      autofillSiteLabel: "Auto-show on this site",
  ...
  const autofillToggle = document.getElementById('autofill-site-toggle');
  const autofillLabel = document.getElementById('autofill-site-label');
  ...
  // --- Per-site auto-show toggle (mirrors content.js suppression) ---
  function getActiveOrigin() {
    return new Promise((resolve) => {
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        const url = tabs && tabs[0] && tabs[0].url;
        if (!url) { resolve(null); return; }
        try { resolve(new URL(url).origin); } catch (e) { resolve(null); }
      });
    });
  }

  function isFillableOrigin(origin) {
    return !!origin && /^(https?:|file:)\/\//.test(origin);
  }

  function getSuppressedMap() {
    return new Promise((resolve) => {
      chrome.storage.local.get(AUTOFILL_SUPPRESS_KEY, (res) => {
        resolve((res && res[AUTOFILL_SUPPRESS_KEY]) || {});
      });
    });
  }
  ...
  function applyTranslations(lang) {
    ...
    autofillLabel.textContent = t.autofillSiteLabel;
  ...
  function renderPasswords() {
    ...
    chrome.storage.local.get(null, (items) => {
      ...
      delete credentials[AUTOFILL_SUPPRESS_KEY];
  ...
  function init() {
    ...
    settingsButton.addEventListener('click', async () => {
      ...
      // Per-site auto-show toggle: checked = auto-show ON (not suppressed).
      currentTabOrigin = await getActiveOrigin();
      const suppressed = await getSuppressedMap();
      const fillable = isFillableOrigin(currentTabOrigin);
      autofillToggle.disabled = !fillable;
      autofillToggle.checked = fillable ? !suppressed[currentTabOrigin] : false;
    ...
    autofillToggle.addEventListener('change', async () => {
      ...
      const map = await getSuppressedMap();
      if (autofillToggle.checked) delete map[currentTabOrigin];
      else map[currentTabOrigin] = true;
      await new Promise((r) => chrome.storage.local.set({ [AUTOFILL_SUPPRESS_KEY]: map }, r));
    });
